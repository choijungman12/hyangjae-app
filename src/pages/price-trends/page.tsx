  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 pb-24 overflow-hidden">
      ...
      <section className="px-4 pt-4 relative z-10">
        {(() => {
          const isLive = data?.source === 'garak' || data?.source === 'kamis';
          const sourceLabel =
            data?.source === 'garak' ? '가락시장 실시간 경락가 연동 중'
            : data?.source === 'kamis' ? 'KAMIS 실시간 시세 연동 중'
            : '시세 참고 데이터 (목업)';
          const sourceSub =
            data?.source === 'garak' ? '서울특별시농수산식품공사 OpenAPI 직접 연동'
            : data?.source === 'kamis' ? '가락시장 + 전국 도매시장 경락가 기준'
            : (GARAK_CONFIGURED || KAMIS_CONFIGURED)
              ? '실연동 실패 → 2026년 평균 시세 기반 fallback'
              : '실시간 API 연동 전 · 2026년 평균 시세 기반';
          return (
        <div className={`rounded-2xl px-4 py-3 flex items-center gap-3 ${isLive ? 'bg-emerald-50 border border-emerald-200' : 'bg-amber-50 border border-amber-200'}`}>
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${isLive ? 'bg-emerald-500' : 'bg-amber-500'}`}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className={`text-[11px] ${isLive ? 'text-emerald-700' : 'text-amber-700'}`}>
              {sourceSub}
            </p>
          </div>
          {data && (
